...
static void _lockscreen_sim_lock_view_create(Evas_Object *view)
{
	...
	if (result) {
		...
		handler[0] = ecore_event_handler_add(LOCKSCREEN_EVENT_SIM_LOCK_UNLOCKED,
				_sim_lock_response, (void *)SIM_LOCK_SIM_RESPONSE_UNLOCKED);
		handler[1] = ecore_event_handler_add(LOCKSCREEN_EVENT_SIM_LOCK_INCORRECT,
				_sim_lock_response, (void *)SIM_LOCK_SIM_RESPONSE_PIN_INCORRECT);
		handler[2] = ecore_event_handler_add(LOCKSCREEN_EVENT_SIM_LOCK_BLOCKED,
