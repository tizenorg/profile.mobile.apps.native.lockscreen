...
static void _lockscreen_sim_lock_view_create(Evas_Object *view, int locked_count, int unlock_card_num);
...
static Eina_Bool _sim_lock_response(void *data, int type, void *event_info)
{
	...
	case SIM_LOCK_SIM_RESPONSE_UNLOCKED:
		...
		first_locked_card = -1;
		locked_remaining = lockscreen_sim_lock_pin_required(&first_locked_card);
		lockscreen_password_view_clear(pass_view);

		if (locked_remaining && first_locked_card >= 0)
			_lockscreen_sim_lock_view_create(main_view, result, first_locked_card);
		else
...
int lockscreen_sim_lock_ctrl_init(Evas_Object *view)
{
	...
	main_view = view;
	result = lockscreen_sim_lock_pin_required(&first_locked_card);
