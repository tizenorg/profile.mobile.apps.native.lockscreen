...
{
	lockscreen_event_t *candidate = lockscreen_events_launch_candidate_get();
	...
	if (candidate) {
		lockscreen_event_launch(candidate, NULL);
	}
	return EINA_TRUE;
}
...
static void _lockscreen_device_lock_ctrl_pass_view_cancel_button_clicked(void *data, Evas_Object  *obj, void *event_info)
{
	...
	lockscreen_events_launch_candidate_set(NULL);
}
...
_lockscreen_device_lock_ctrl_unlock_request(void *data, int event, void *event_info)
{
	...
	lockscreen_event_t *ev = lockscreen_events_launch_candidate_get();
