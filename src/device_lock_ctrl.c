...
{
	if (data) lockscreen_event_free(data);
}
...
static int _lockscreen_device_lock_ctrl_unlock_panel_show(lockscreen_device_lock_type_e type, const lockscreen_event_t *event)
{
	...
	if (event) {
		event = lockscreen_event_copy(event);
	}
	...
	evas_object_smart_callback_add(pass_view, SIGNAL_ACCEPT_BUTTON_CLICKED, _lockscreen_device_lock_ctrl_pass_view_accept_button_clicked, event);
	lockscreen_main_view_part_content_set(main_view, PART_PASSWORD, pass_view);
	evas_object_event_callback_add(pass_view, EVAS_CALLBACK_DEL, _lockscreen_device_lock_ctrl_view_del, event);
