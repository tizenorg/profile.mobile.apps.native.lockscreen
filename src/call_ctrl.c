...
static void _call_icon_view_update(void)
{
	...
	int call_active = lockscreen_call_active_is();
...
{
	Evas_Object *cam_view = lockscreen_main_view_part_content_get(main_view, PART_CALL);
	if (cam_view)
...
int lockscreen_call_ctrl_init(Evas_Object *win, Evas_Object *view)
{
	...
	evas_object_smart_callback_add(win, "normal", _lockscreen_call_ctrl_win_normal, NULL);
