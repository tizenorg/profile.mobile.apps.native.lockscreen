...
static void _call_icon_view_update(void)
{
	...
	if (call_icon_view) {
		if (call_active)
			lockscreen_swipe_icon_text_set(call_icon_view, "Return to call");
		else
			lockscreen_swipe_icon_text_set(call_icon_view, "Emergency call");
	}
...
int lockscreen_call_ctrl_init(Evas_Object *view)
{
	...
	call_icon_view = lockscreen_swipe_icon_view_create(view, ICON_CALL);
	if (!call_icon_view) {
		DBG("call_icon_view == NULL");
		ecore_event_handler_del(handler);
		return 1;
	}

	lockscreen_main_view_part_content_set(view, PART_CALL, call_icon_view);
	_call_icon_view_update();
