...
call_app_launch_request_reply_cb(app_control_h request, app_control_h reply, app_control_result_e result, void *user_data)
{
	switch (result) {
		...
		case APP_CONTROL_RESULT_SUCCEEDED:
			DBG("Call application launch succcessed.");
			break;
...
int lockscreen_call_app_launch_request(void)
{
	...
	if (lockscreen_call_status_active())
		ret = app_control_set_operation(control, CALL_APP_CONTROL_OPERATION_RETURN);
	else
		ret = app_control_set_operation(control, CALL_APP_CONTROL_OPERATION_EMERGENCY);
...
/*
	ret = app_control_set_operation(control, APP_CONTROL_OPERATION_DIAL);
	if (ret != APP_CONTROL_ERROR_NONE) {
		ERR("app_control_set_operation failed");
		return -1;
	}
*/
	...
	DBG("Launch request send for %s", CALL_APP_CONTROL_OPERATION_RETURN);
