...
static Evas_Event_Flags _swipe_state_end(void *data, void *event_info)
{
	...
	if (elm_genlist_at_xy_item_get(pd->genlist, li->momentum.x1, li->momentum.y1, NULL) &&
			li->angle > 130.0 && li->angle < 220.0) {
...
_lockscreen_events_view_page_create(Evas_Object *events_view)
{
	...
	elm_gesture_layer_flick_time_limit_ms_set(pd->gesture_layer, 500);
...
lockscreen_events_view_event_miniature_create(Evas_Object *parent, const lockscreen_event_t *event)
{
	...
	if (!lockscreen_event_icon_get(event) ||
		!elm_image_file_set(icon, lockscreen_event_icon_get(event), NULL)) {
			elm_image_file_set(icon, util_get_shared_res_file_path(DEFAULT_IMAGE_PATH), NULL);
		}
	...
	if (lockscreen_event_sub_icon_get(event)) {
		icon = elm_icon_add(ret);
		if (!elm_image_file_set(icon, lockscreen_event_sub_icon_get(event), NULL)) {
