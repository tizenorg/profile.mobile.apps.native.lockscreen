...
static char *background_file_backup = NULL;
...
static void _lockscreen_background_remote_file_set(char *file)
{
	...
	if (!strcmp(file, "bg/unset")) {
		...
		background_file = strdup(background_file_backup);
	...
	if (file) {
		...
		free(background_file_backup);
		background_file_backup = strdup(background_file);
...
void lockscreen_background_shutdown(void)
{
	if (init_count) {
		...
		free(background_file);
		free(background_file_backup);
		free(background_runtime_file);

		background_file = NULL;
		background_file_backup = NULL;
