...
static void _sim_lock_response_cb(TapiHandle *handle, int result, void *data, void *user_data)
{
	...
	if (result == TAPI_SIM_PIN_OPERATION_SUCCESS)
		ecore_event_add(LOCKSCREEN_EVENT_SIM_LOCK_UNLOCKED, NULL, NULL, NULL);
	else if (result == TAPI_SIM_PIN_INCORRECT_PASSWORD || result == TAPI_SIM_PUK_INCORRECT_PASSWORD) {
		ERR("Wrong pin");
...
void lockscreen_sim_lock_unlock(const char *pass)
{
	...
	case TAPI_SIM_STATUS_SIM_PIN_REQUIRED:
		...
		ret = tel_verify_sim_pins(sim_card[0].handle, &pin_data, _sim_lock_response_cb, NULL);
		break;
	case TAPI_SIM_STATUS_SIM_PUK_REQUIRED:
		pin_data.type = TAPI_SIM_PTYPE_PUK1;
		ret = tel_verify_sim_puks(sim_card[0].handle, &pin_data, NULL, _sim_lock_response_cb, NULL);
		break;
	...
	if (!init_count) {
		...
		for (i = 0; i < slot_count; ++i) {
			...
			if (sim_card[i].handle)
				tel_register_noti_event(sim_card[i].handle, TAPI_NOTI_SIM_STATUS, _card_status_changed_cb, &sim_card[i]);
