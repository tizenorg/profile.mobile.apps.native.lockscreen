...
static int _lockscreen_device_lock_dpm_init(void)
{
	...
	if (ret != DPM_ERROR_NONE) {
		ERR("dpm_password_get_maximum_failed_attempts_for_wipe failed");
		dpm_context_release_password_policy(handle, password_policy_handle);
		dpm_context_destroy(handle);
		return -1;
	}
...
_lockscreen_device_unlock_with_password(const char *pass, int *attempts_left)
{
	*attempts_left = -1;
	...
	if (ret == AUTH_PASSWD_API_SUCCESS) {
		...
		if (ret == AUTH_PASSWD_API_ERROR_PASSWORD_MISMATCH) {
			if (attempt == max_attempts) {
				if (_lockscreen_device_lock_dpm_status_set(DPM_PASSWORD_STATUS_MAX_ATTEMPTS_EXCEEDED))
	...
	ERR("auth_passwd_check_passwd_state error: %d", ret);
	return LOCKSCREEN_DEVICE_UNLOCK_ERROR;
}
