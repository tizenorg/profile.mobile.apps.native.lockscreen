...
static void _lockscreen_device_vconf_idle_key_changed(keynode_t *node, void *user_data)
{
	if ((node->value.i == VCONFKEY_IDLE_UNLOCK) &&
			lockscreen_device_lock_type_get() == LOCKSCREEN_DEVICE_LOCK_NONE)
...
{
	unsigned int current_attempt, max_attempts, expiere_sec;

	int ret = auth_passwd_check_passwd_state(AUTH_PWD_NORMAL, &current_attempt, &max_attempts, &expiere_sec);
	switch (ret) {
	...
	ret = auth_passwd_check_passwd(AUTH_PWD_NORMAL, pass, &current_attempt, &max_attempts, &expiere_sec);
	switch (ret) {
		case AUTH_PASSWD_API_SUCCESS:
		case AUTH_PASSWD_API_ERROR_NO_PASSWORD:
			return LOCKSCREEN_DEVICE_UNLOCK_SUCCESS;
		...
		case AUTH_PASSWD_API_ERROR_PASSWORD_MAX_ATTEMPTS_EXCEEDED:
			DBG("auth_passwd_check_passwd : %d %d %d", current_attempt, max_attempts, expiere_sec);
			*attempts_left = max_attempts - current_attempt;
			if (current_attempt == max_attempts && !_lockscreen_device_lock_dpm_status_set(DPM_PASSWORD_STATUS_MAX_ATTEMPTS_EXCEEDED))
