...
#define PIN_PASS_LEN_KEY "__pass_len_key"
...
_lockscreen_password_view_pin_entry_activated(void *data, Evas_Object *obj, void *event_info)
{
	...
	int len = (int)evas_object_data_get(data, PIN_PASS_LEN_KEY);
...
static Evas_Object* _lockscreen_password_view_pin_create(Evas_Object *parent)
{
	...
	_lockscreen_password_pin_button_setup(ly, 4, 1, "", "" , _lockscreen_password_view_accept_button_clicked);
...
{
	evas_object_data_set(view, PIN_PASS_LEN_KEY, (void*)length);
}
